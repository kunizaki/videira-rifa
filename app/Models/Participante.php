...
class Participante extends Model
{
    ...
    public function is_premiada() {

        $admin = User::find(1);

        $numeros_premiados = $this->numbers();
        $numeros_comprados = array_filter(array_map('trim', explode(',', $this->rifa()->premiada)));

        $numeros_ganhadores = array_intersect($numeros_premiados, $numeros_comprados);
        
        if (!empty($numeros_ganhadores)) {
            $quantidade_premiados = count($numeros_ganhadores);
            $msg = "Ganhei a(s) Cota(s) Premiada(s) " . implode(', ', $numeros_ganhadores);
            $resultado = (object) [
                'ganhou' => true,
                'numeros' => $numeros_ganhadores,
                'link' => $this->linkWppPremiada($admin->telephone, $msg)
    ...
    public function linkWppPremiada($phone, $message)
    {
        $tel = "55" . preg_replace('/\D/', '', $phone);
        if (strlen($tel) >= 12) {
            $encodedMessage = urlencode($message);
            $link = 'https://wa.me/' . $tel . '?text=' . $encodedMessage;
