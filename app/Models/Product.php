...
class Product extends Model
{
    ...
    {
        return $this->hasOne(Upsell::class, 'product_id', 'id');
    }
        ...
        switch ($this->status) {
            case 'Ativo':
                if ($this->porcentagem() >= 80) {
                    $status = '<span class="badge mt-1 blink" style="color: #fff; background-color: #FF0000; text-transform: uppercase;">Corre que está acabando!</span>';
                } else {
                    $status = '<span class="badge mt-1 bg-success blink">Adquira já!</span>';
                }
                break;
            case 'Acabando':
                    $status = '<span class="badge mt-1 blink" style="color: #fff; background-color: #FF0000; text-transform: uppercase;">Corre que está acabando!</span>';
            ...
            case 'Finalizado':
                if ($this->premios()->where('descricao', '!=', '')->where('ganhador', '!=', '')->count() == 0) {
                    $status = '<span class="badge mt-1 blink" style="color: #fff; background-color: rgba(0,0,0,.7);">Aguarde sorteio!</span>';
                } else {
                    $status = '<span class="badge mt-1 bg-danger">Finalizado</span>';
                }
