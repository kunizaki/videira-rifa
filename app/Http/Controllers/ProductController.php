...
class ProductController extends Controller
{
    ...
    public function product($slug, $tokenAfiliado = null)
    {
        ...
        $upsells = Upsell::where('product_id', $productID)->first();
        ...
        $productData = DB::table('products')
            ->select('premiada', 'premiada_win', 'descricaopremiada')
            ->where('id', $productID)
            ->first();

        if ($productData && isset($productData->premiada_win)) {
            $windata = explode(',', $productData->premiada_win);
        } else {
            $windata = [];
        }

        if ($productData && isset($productData->premiada)) {
            $pdata = explode(',', $productData->premiada);
        } else {
            $pdata = [];
        }

        $result_product = (object) [
            'num' => $pdata,
            'win' => $windata,
            'descricao' => $productData->descricaopremiada
        ];
    ...
    public function UpsellImpl(Request $request) {

        $upsell = Upsell::where('product_id',  $request->productID)->first();
        $product = ModelsProduct::find($request->productID);

        $num_rifas_total = $request->qtdNumbers;

        if ($num_rifas_total < $product->minimo) {
            return Redirect::back()->withErrors('Você precisa comprar no mínimo ' . $product->minimo . ' números');
        }
        if ($num_rifas_total > $product->maximo) {
        ...
        $data = [
            ...
            'qtdNumbers' => $request->qtdNumbers,
            'productName' => $request->productName,
            'productID' => $request->productID,
            'numberSelected' => $request->numberSelected,
    ...
    public function bookProductManualy(Request $request)
    {
        ...
        try {
            ...
            if($request->has('upsellcheck') && $upsell) {    
                $num_rifas_total += $upsell->qtdNumeros;
            }
            ...
            if (!$integracaoGateway) {
                ...
                if ($statusProduct->status == "Ativo" or $statusProduct->status == "Acabando") {
                    ...
                    if (str_starts_with($prod->modo_de_jogo, 'fazendinha')) {
                        ...
                        if ($prod->type_raffles == 'manual' || $prod->type_raffles == 'mesclado') {
                            ...
                            if (isset($randomNumbers)) {
                                if (count($resutlNumbers) < $prod->minimo) {
                                    return Redirect::back()->withErrors('Você precisa comprar no mínimo ' . $prod->minimo . ' números');
                                }
                                if (count($resutlNumbers) > $prod->maximo) {
                                    return Redirect::back()->withErrors('Você só pode comprar no máximo ' . $prod->maximo . ' números');
                                }
                            } else {
                                if (count($resutlNumbers) < $prod->minimo) {
                                    return Redirect::back()->withErrors('Você precisa comprar no mínimo ' . $prod->minimo . ' números');
                            ...
                            $selecionados = array_slice($disponiveis, 0, $num_rifas_total);

                            if (count($disponiveis) < $num_rifas_total) {
                    ...
                    if (isset($randomNumbers)) {
                        ...
                        if ($request->qtdNumbers < $product->minimo) {
                            return Redirect::back()->withErrors('Você precisa comprar no mínimo ' . $product->minimo . ' números');
                        }
                        if ($request->qtdNumbers > $product->maximo) {
                            return Redirect::back()->withErrors('Você só pode comprar no máximo ' . $product->maximo . ' números');
                        }
                    }
                    $order_sem_upsell = count($resutlNumbers);
                    $price_upsell = 0;

                    if($request->has('upsellcheck') && $upsell) {
                        $order_sem_upsell -= $upsell->qtdNumeros;
                        $price_upsell = str_replace(",", ".", $upsell->valor);
                    }
                    
                    $promocoes = $prod->promocoes()->where('qtdNumeros', '>', 0);
                    
                    $percentDesconto = 0;
                    if($promocoes->count() > 0) {
                        foreach ($promocoes as $promo) {
                            if($order_sem_upsell >= $promo->qtdNumeros) {
                                $percentDesconto =  $promo->desconto;
                            }
                        }
                    }
                    ...
                    $price = $order_sem_upsell * $new;
                    if($percentDesconto > 0) {
                        $desconto = $price * ($percentDesconto /100);
                    }
                    
                    $result_desconto = $price - $desconto;
                    
                    $total_final = $result_desconto + $price_upsell;
                    // dd($price_upsell);


                    $resultPrice = number_format($total_final, 2, ",", ".");
                    $resultPricePIX = number_format($total_final, 2, ".", ",");
